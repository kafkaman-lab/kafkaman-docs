Newman is a command-line Collection Runner for kafkaman. It enables you to run and test a kafkaman Collection directly from the command line. It's built with extensibility in mind so that you can easily integrate it with your continuous integration servers and build systems.

Newman maintains feature parity with kafkaman and allows you to run collections the way they are executed inside the collection runner in kafkaman.

Newman resides in the [NPM registry](https://www.npmjs.com/package/newman) and on [GitHub](https://github.com/kafkamanlabs/newman).

[![newman gif](https://assets.postman.com/kafkaman-docs/newman-running-in-terminal.gif)](https://assets.postman.com/kafkaman-docs/newman-running-in-terminal.gif)
...
```bash
$ newman run kafkamanCollection.json -e environment.json --bail
```
...
**Note:** Newman allows you to use all [libraries and objects](/docs/writing-scripts/script-references/kafkaman-sandbox-api-reference/) that kafkaman supports to run tests and pre-request scripts.
...
```bash
$ newman run file-upload.kafkaman_collection.json
```
...
function (emitter, reporterOptions, collectionRunOptions) {
  // emitter is is an event emitter that triggers the following events: https://github.com/kafkamanlabs/newman#newmanrunevents
  // reporterOptions is an object of the reporter specific options. See usage examples below for more details.
  // collectionRunOptions is an object of all the collection run options:
  // https://github.com/kafkamanlabs/newman#newmanrunoptions-object--callback-function--run-eventemitter
};
```
...
For the complete documentation, see the [Newman README](https://github.com/kafkamanlabs/newman).
